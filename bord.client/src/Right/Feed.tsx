    ...
    return (
        <>
            <div className="scrollbar-thin h-screen w-1/3 border-l-2 border-slate-700 border-inset bg-slate-800 text-gray-200 overflow-y-scroll">
                <Profile user={user} />
                <button onClick={createFeedPost} className="z-40 shadow-xl opacity-40 absolute bottom-5 right-10 p-3 px-5 bg-sky-500 rounded-full text-xl font-bold hover:opacity-100">+</button>
                {posts.slice().reverse().map((post, index) =>
                    <FeedPost key={index} user={user} content={post.postContent} />
                )}
