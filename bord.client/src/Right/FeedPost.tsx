...
function FeedPost({ user, content, postId }: FeedProps) {

    const [feedModal, setModal] = useState(false);
    ...
    return (
        <>
            <div className="relative grid grid-cols-[10%_90%] border min-h-24 h-fit border-slate-700" >
                ...
                <div className="p-3 overflow-hidden break-words">
                    ...
                    <div className="flex justify-between border-t border-slate-600 w-full my-1">
                        <button onClick={openModal} className="text-sm border border-slate-600 mt-2 rounded-full px-2 hover:bg-slate-500">O</button>
            ...
            {feedModal &&
                <FeedPostModal user={user} content={content} postId={postId} open={feedModal} onClose={() => setModal(false)} />}
        </>
    );
}
